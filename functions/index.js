...
// ✅ NOTIFY: Reply to a well wish
exports.onNewWishReply = onDocumentCreated("messages/{messageId}/replies/{replyId}", async (event) => {
  const data = event.data.data();
  const author = data.author || "Someone";
  await sendToAll(
    `💬 ${author} replied to a well wish`,
    data.text ? data.text.substring(0, 100) : "New reply!",
    { type: "reply" }
  );
});

// ✅ NOTIFY: Reply to an update
exports.onNewUpdateReply = onDocumentCreated("updates/{updateId}/replies/{replyId}", async (event) => {
  const data = event.data.data();
  const author = data.author || "Someone";
  await sendToAll(
    `💬 ${author} replied to an update`,
    data.text ? data.text.substring(0, 100) : "New reply!",
    { type: "reply" }
  );
});
