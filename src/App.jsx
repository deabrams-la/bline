...
const ReplyThread=memo(({parentId,parentCollection,userName})=>{
  ...
  return(
    <div style={{marginTop:6}}>
      {replies.length>0&&(
        <div style={{paddingLeft:12,borderLeft:"2px solid #F0E0E8",marginBottom:4}}>
          {replies.map(r=>(
      ...
      {showInput?(
        <div style={{display:"flex",gap:6,marginTop:4,paddingLeft:replies.length>0?12:0}}>
        ...
        <button onClick={()=>{setShowInput(true);setTimeout(()=>inputRef.current?.focus(),100)}} style={{background:"none",border:"none",fontSize:12,color:"#B2BEC3",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontWeight:600,padding:0,display:"flex",alignItems:"center",gap:4}}>
          💬 Reply
        </button>
      )}
    </div>
...
export default function App(){
  ...
  const TH=settings.totalHerceptin||26;
  ...
  // Herceptin: 12 weekly during chemo, then every 3 weeks for remaining 14 = ~52 weeks total
  const hEnd=new Date(startDate);hEnd.setDate(hEnd.getDate()+364);
  const dH=Math.max(0,Math.ceil((hEnd-now)/(864e5)));
  // Calculate next herceptin date
  const getNextH=(count)=>{
    const d=new Date(startDate);
    if(count<12){d.setDate(d.getDate()+count*7)}
    else{d.setDate(d.getDate()+11*7+(count-11)*21)}
    return d;
  };
  const nextH=getNextH(hMonth);
  const dNextH=Math.max(0,Math.ceil((nextH-now)/(864e5)));
    ...
    <div>
      ...
      <div style={{...S.card,background:"linear-gradient(145deg,#FFF0E8 0%,#FDE8F0 40%,#F0E8F8 100%)",border:"2px solid rgba(232,75,138,0.12)",padding:"22px 20px",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{padding:"14px 16px",borderRadius:15,background:"rgba(255,255,255,0.7)",border:"1px solid rgba(38,166,154,0.1)"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
            <div><div style={{fontFamily:"'Playfair Display',serif",fontSize:20,fontWeight:800,color:"#2D3436"}}>💚 Herceptin</div><div style={{fontSize:12,color:"#999",marginTop:1}}>{hMonth<TH?(hMonth<12?"Weekly with chemo":"Every 3 weeks"):"Complete! ✨"}{hMonth<TH&&(dNextH<=0?" · Treatment day! 💪":` · Next in ${dNextH}d`)}</div></div>
            <div style={{textAlign:"right"}}><div style={{fontSize:22,fontWeight:800,color:"#26A69A"}}>{hMonth}/{TH}</div><div style={{fontSize:11,color:"#999"}}>treatments</div></div>
          </div>
